...
- [第负一部分：文件结构介绍](#第负一部分文件结构介绍)
- [第零部分：环境准备](#第零部分环境准备)
- [第一部分：MIMIC-III 数据提取（SQL）](#第一部分mimic-iii-数据提取sql)
  - [步骤一：提取视图](#步骤一提取视图)
  - [步骤二：提取 CSV 文件](#步骤二提取-csv-文件)
- [第二部分：数据准备工作](#第二部分数据准备工作)
  - [1. 结构化数据（时间序列）的预处理与数据集构建](#1-结构化数据时间序列的预处理与数据集构建)
  - [2. 非结构化文本（如临床笔记）的预处理与数据集构建](#2-非结构化文本如临床笔记的预处理与数据集构建)
  - [3. 下载预训练模型](#3-下载预训练模型)
- [第三部分：训练模型(不含 BERT 微调)](#第三部分训练模型不含-bert-微调)
  - [1.使用 ClinicalBERT 对文本进行词嵌入](#1使用-clinicalbert-对文本进行词嵌入)
  - [2. 训练模型](#2-训练模型)
- [第五部分：训练模型（包含 BERT 微调，请准备好足够的计算资源）](#第五部分训练模型包含-bert-微调请准备好足够的计算资源)
- [第六部分：使用大模型总结文本](#第六部分使用大模型总结文本)

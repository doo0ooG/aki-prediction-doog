...
) TO 'data\icustay_detail-kdigo_stages_measured.csv' 
WITH CSV HEADER DELIMITER ';';
...
) TO 'data\vitals-kdigo_stages_measured.csv' 
WITH CSV HEADER DELIMITER ';';
...
) TO 'data\labs-kdigo_stages_measured.csv' 
WITH CSV HEADER DELIMITER ';';
...
COPY (
    SELECT ve.icustay_id AS icustay_id, 
           ve.charttime AS charttime, 
           vent, 
           vasopressor, 
           sedative 
    FROM mimiciii.vent_kdigo_stages_labs_vitals_charttime ve
    JOIN mimiciii.vasopressor_kdigo_stages_labs_vitals_charttime va 
      ON ve.icustay_id = va.icustay_id AND ve.charttime = va.charttime
    JOIN mimiciii.sedatives_kdigo_stages_labs_vitals_charttime s 
      ON va.icustay_id = s.icustay_id AND va.charttime = s.charttime
...
) TO 'data\vents-vasopressor-sedatives-kdigo_stages_measured.csv' 
WITH CSV HEADER DELIMITER ';';